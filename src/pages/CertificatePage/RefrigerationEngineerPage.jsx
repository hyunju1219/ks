import React from 'react';
import { Helmet } from 'react-helmet';
import { Link } from 'wouter';
import SubpageHeader from '../../components/SubpageHeader/SubpageHeader';
import CTASection from '../../components/CTASection/CTASection';
import * as S from './style';

const RefrigerationEngineerPage = () => {
  const breadcrumbs = [
    { name: '자격증 소개', link: '/certificate' },
    { name: '공조냉동기계', link: '/certificate/refrigeration' },
    { name: '기사', link: null }
  ];

  return (
    <S.PageContainer>
      <Helmet>
        <title>공조냉동기계 기사 - 금성기술직업전문학교</title>
        <meta name="description" content="공조냉동기계 기사 자격증 취득 안내. 시험일정, 응시자격, 시험과목 등 상세 정보 제공" />
      </Helmet>
      
      <SubpageHeader 
        title="공조냉동기계 기사" 
        subtitle="냉동공조 시스템의 계획, 설계, 시공, 관리 분야의 전문 기술인 양성"
        breadcrumbs={breadcrumbs}
      />
      
      <S.ContentSection>
        <S.SectionInner>
          {/* 상단 네비게이션 */}
          <S.CertificateNav>
            <div>
              <Link href="/certificate/refrigeration">
                <S.CertificateNavItem>개요</S.CertificateNavItem>
              </Link>
            </div>
            <div>
              <Link href="/certificate/refrigeration/master">
                <S.CertificateNavItem>기능사</S.CertificateNavItem>
              </Link>
            </div>
            <div>
              <Link href="/certificate/refrigeration/industrial">
                <S.CertificateNavItem>산업기사</S.CertificateNavItem>
              </Link>
            </div>
            <div>
              <Link href="/certificate/refrigeration/engineer">
                <S.CertificateNavItem isActive={true}>기사</S.CertificateNavItem>
              </Link>
            </div>
          </S.CertificateNav>
          
          {/* 상단 배너 이미지 */}
          <div style={{
            width: '100%',
            marginBottom: '2rem',
            borderRadius: '0.5rem',
            overflow: 'hidden',
            boxShadow: '0 4px 8px rgba(0, 0, 0, 0.1)',
            border: '1px solid #e5e7eb'
          }}>
            <img 
              src="https://images.unsplash.com/photo-1494412574643-ff11b0a5c1c3?ixlib=rb-4.0.3&auto=format&fit=crop&w=1200&h=350" 
              alt="공조냉동기계 기사"
              style={{
                width: '100%',
                height: 'auto',
                maxHeight: '350px',
                objectFit: 'cover',
                display: 'block'
              }} 
            />
          </div>
          
          {/* 타이틀 및 설명 */}
          <div style={{ marginBottom: '2rem' }}>
            <S.CertificateTitle>공조냉동기계 기사</S.CertificateTitle>
            <S.CertificateDescription>
              공조냉동기계 기사는 냉동공조 시스템을 계획, 설계, 시공, 유지보수하는 고급 전문인력을 검증하는 자격증입니다. 
              냉동 및 공조 설비의 종합적인 이해와 복합적인 문제해결 능력을 기반으로 대규모 시스템의 설계와 운영을 
              담당할 수 있는 전문 기술인력을 양성합니다.
            </S.CertificateDescription>
          </div>
          
          {/* 응시자격 */}
          <S.InfoSection>
            <S.InfoTitle>응시자격</S.InfoTitle>
            <S.FlexColumnDesktop>
              <S.DetailItem>
                <S.DetailTitle>학력</S.DetailTitle>
                <S.DetailContent>
                  <p>- 관련학과 4년제 대학 졸업자</p>
                  <p>- 관련학과 전문대학 졸업 후 실무경력 1년 이상인 자</p>
                  <p>- 관련학과 기능대학 전문학사 이상의 학위 취득자</p>
                  <p>- 다른 법령에 의해 위와 동등 이상의 학력이 인정되는 자</p>
                </S.DetailContent>
              </S.DetailItem>
              
              <S.DetailItem>
                <S.DetailTitle>경력</S.DetailTitle>
                <S.DetailContent>
                  <p>- 관련 실무 경력 4년 이상인 자</p>
                  <p>- 산업기사 자격 취득 후 실무 경력 1년 이상인 자</p>
                  <p>- 기능사 자격 취득 후 실무 경력 3년 이상인 자</p>
                  <p>- 학력 + 경력(실무 + 교육훈련) 조건 만족자</p>
                </S.DetailContent>
              </S.DetailItem>
            </S.FlexColumnDesktop>
          </S.InfoSection>
          
          {/* 시험과목 */}
          <S.InfoSection>
            <S.InfoTitle>시험과목</S.InfoTitle>
            <S.FlexColumnDesktop>
              <S.DetailItem>
                <S.DetailTitle>필기시험</S.DetailTitle>
                <S.DetailContent>
                  <p>- <strong>과목수</strong>: 4과목</p>
                  <p>- <strong>시험시간</strong>: 2시간 30분</p>
                  <p>- <strong>문제수</strong>: 과목당 20문제(총 80문항, 객관식 4지선다)</p>
                  <p>- <strong>합격기준</strong>: 과목당 40점 이상, 전 과목 평균 60점 이상</p>
                  <p>- <strong>과목명</strong>:</p>
                  <ul>
                    <li>냉동공학</li>
                    <li>냉동장치</li>
                    <li>냉방장치</li>
                    <li>공기조화장치</li>
                  </ul>
                </S.DetailContent>
              </S.DetailItem>
              
              <S.DetailItem>
                <S.DetailTitle>실기시험</S.DetailTitle>
                <S.DetailContent>
                  <p>- <strong>검정방법</strong>: 작업형</p>
                  <p>- <strong>시험시간</strong>: 5시간 내외</p>
                  <p>- <strong>합격기준</strong>: 60점 이상(100점 만점)</p>
                  <p>- <strong>주요 평가내용</strong>:</p>
                  <ul>
                    <li>냉동 및 공조 시스템 설계 및 계산</li>
                    <li>복합 냉동시스템의 구성 및 분석</li>
                    <li>대형 공조시스템 설계 및 평가</li>
                    <li>에너지 효율 분석 및 시스템 최적화</li>
                    <li>냉동공조 시스템 종합 설계</li>
                    <li>고급 제어 시스템 구성 및 응용</li>
                  </ul>
                </S.DetailContent>
              </S.DetailItem>
            </S.FlexColumnDesktop>
          </S.InfoSection>
          
          {/* 시험일정 */}
          <S.InfoSection>
            <S.InfoTitle>2023년 시험일정</S.InfoTitle>
            <S.ExamScheduleTable>
              <thead>
                <tr>
                  <th>회차</th>
                  <th>필기시험 원서접수</th>
                  <th>필기시험</th>
                  <th>필기 합격발표</th>
                  <th>실기시험 원서접수</th>
                  <th>실기시험</th>
                  <th>최종 합격발표</th>
                </tr>
              </thead>
              <tbody>
                <tr>
                  <td>1회</td>
                  <td>01.24 ~ 01.27</td>
                  <td>02.13 ~ 03.01</td>
                  <td>03.08</td>
                  <td>03.13 ~ 03.16</td>
                  <td>04.15 ~ 04.28</td>
                  <td>05.12</td>
                </tr>
                <tr>
                  <td>2회</td>
                  <td>03.29 ~ 04.01</td>
                  <td>04.22 ~ 05.07</td>
                  <td>05.12</td>
                  <td>05.16 ~ 05.19</td>
                  <td>06.24 ~ 07.09</td>
                  <td>07.21</td>
                </tr>
                <tr>
                  <td>3회</td>
                  <td>07.10 ~ 07.13</td>
                  <td>08.05 ~ 08.20</td>
                  <td>08.25</td>
                  <td>08.28 ~ 08.31</td>
                  <td>10.07 ~ 10.20</td>
                  <td>11.03</td>
                </tr>
              </tbody>
            </S.ExamScheduleTable>
            <S.ExamInfoNote>
              * 상기 일정은 해당 연도의 공식 일정이며, 시행기관의 사정에 따라 변경될 수 있습니다. 정확한 일정은 <a href="https://www.q-net.or.kr" target="_blank" rel="noopener noreferrer">Q-Net</a> 홈페이지에서 확인하시기 바랍니다.
            </S.ExamInfoNote>
          </S.InfoSection>
          
          {/* 취업 분야 */}
          <S.InfoSection>
            <S.InfoTitle>취업 분야</S.InfoTitle>
            <S.CareerList>
              <S.CareerItem>대규모 냉동공조 시스템 설계 및 시공업체</S.CareerItem>
              <S.CareerItem>대형 건물 및 시설물 냉동공조 설비 관리자</S.CareerItem>
              <S.CareerItem>공공기관 및 대기업 설비 담당 엔지니어</S.CareerItem>
              <S.CareerItem>에너지 관리 및 설비 최적화 컨설턴트</S.CareerItem>
              <S.CareerItem>냉동공조 장비 제조업체 설계 및 R&D 부서</S.CareerItem>
              <S.CareerItem>대형 프로젝트 현장 관리 및 감독</S.CareerItem>
              <S.CareerItem>환경 및 에너지 절약 관련 컨설팅</S.CareerItem>
            </S.CareerList>
          </S.InfoSection>
          
          {/* 교육 과정 */}
          <S.InfoSection>
            <S.InfoTitle>금성기술직업전문학교 교육 과정</S.InfoTitle>
            <S.CourseInfo>
              <S.CourseItem>
                <S.CourseTitle>공조냉동기계 기사 취득 과정</S.CourseTitle>
                <S.CourseDetails>
                  <p>- <strong>교육기간</strong>: 6개월</p>
                  <p>- <strong>교육인원</strong>: 12명</p>
                  <p>- <strong>교육내용</strong>:</p>
                  <ul>
                    <li>고급 냉동 및 공조 이론</li>
                    <li>대형 냉동설비 시스템 설계</li>
                    <li>복합 공조시스템 설계 및 해석</li>
                    <li>시스템 통합 및 자동제어</li>
                    <li>에너지 효율화 및 최적 운전 기법</li>
                    <li>시스템 설계 프로젝트 실습</li>
                    <li>시공 및 감리 기술</li>
                    <li>HVAC 시스템 시뮬레이션</li>
                    <li>실전 문제풀이 및 실기 심화 훈련</li>
                  </ul>
                </S.CourseDetails>
              </S.CourseItem>
            </S.CourseInfo>
          </S.InfoSection>
          
          {/* 교육과정 링크 */}
          <div style={{ marginTop: '2rem', marginBottom: '2rem', textAlign: 'center' }}>
            <Link href="/course">
              <S.CourseButton>관련 교육과정 보기</S.CourseButton>
            </Link>
          </div>
        </S.SectionInner>
      </S.ContentSection>
      
      <CTASection />
    </S.PageContainer>
  );
};

export default RefrigerationEngineerPage;